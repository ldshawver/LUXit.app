...
SET after_hours_text = COALESCE(
        NULLIF(pn.after_hours_text, ''),
        NULLIF((
            SELECT ps.after_hours_sms_body
            FROM phone_settings ps
            WHERE ps.company_id = pn.company_id
            LIMIT 1
        ), ''),
        NULLIF(ta.after_hours_text, ''),
        'Thanks for reaching out. Our business hours are daily from 2 PM to 2 AM. We’ll respond as soon as we’re back online.'
    ),
